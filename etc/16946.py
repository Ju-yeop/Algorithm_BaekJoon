...
for p in range(N):
    for q in range(M):
        if mrx[p][q] == 0 and area[p][q] == 0:
            ...
            area_value[area_count] = temp    #area matrix와 result matrix를 연결 해줄 area_Value dict 선언
            area_count += 1
...
for i in area_value.keys():   #여기서 너무 오래걸리는거 같은데..
    for j in range(N):
        for v in range(M):
            if area[j][v] == i:
...
def bfs(x, y, area_count):
    ...
    while True:
        if len(ls) == 0:
            break
        new_x, new_y = ls.popleft()
        zero_list.append([new_x, new_y, area_count])
        for i in range(4):
            move_x = new_x + move[i][0]
            move_y = new_y + move[i][1]
            if (-1 < move_x < N) and (-1 < move_y < M):
...
for p in range(N):
    for q in range(M):
        if mrx[p][q] == 0 and visited[p][q] is False:
            zero_list = bfs(p, q, area_count)
            for i in range(len(zero_list)):
                result[zero_list[i][0]][zero_list[i][1]] = len(zero_list)    #0인 부분을 연결되어있는 개수로 바꿔
                area[zero_list[i][0]][zero_list[i][1]] = area_count
            area_count += 1
    ...
    for j in range(M):
        ...
        if result[i][j] == 0:
            ...
            for v in range(4):
                ...
                n_y = y + move[v][1]
                if (-1 < n_x < N) and (-1 < n_y < M):
                    if result[n_x][n_y] != 0 and area[n_x][n_y] not in area_judge:
                        final[i][j] += result[n_x][n_y]
                        area_judge.append(area[n_x][n_y])
            final[i][j] += 1
            final[i][j] %= 10
